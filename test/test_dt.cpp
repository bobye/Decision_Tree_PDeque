...
struct Metric {
    real_t FP, FN, Sensitivity, Specificity, Accuracy;
};

template<typename LabelType>
Metric metric(LabelType *y_pred, LabelType *y_true, size_t n) {
    ...
    return {(real_t) fp / (real_t) (fp + tp),
            (real_t) fn / (real_t) (fn + tn),
            (real_t) tp / (real_t) (tp + fn),
            (real_t) tn / (real_t) (tn + fp),
            (real_t) (tp + tn) / (real_t) (tp + tn + fp + fn)};
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 1) {
        ...
        test_fs.close();
//        classifier->predict_debug(X);
        classifier->predict(X, M, y_pred);
    }

    auto result = metric(y_pred, y, M);
    printf("test metric: FP %.3f, FN %.3f, Sensitivity %.3f, Specificity %.3f, Accuracy %.3f\n",
           result.FP, result.FN, result.Sensitivity, result.Specificity, result.Accuracy);
